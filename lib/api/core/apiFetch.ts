...
export const apiFetch = async (endpoint: string, options: ApiFetchOptions = {}): Promise<any> => {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      console.log(`API Request (attempt ${attempt + 1}/${maxRetries}):`, endpoint);
      ...
      if (attempt === maxRetries - 1 || !(error instanceof TypeError) || (error as any)?.message !== 'Network request failed') {
        // Handle network errors specifically
        if (error instanceof TypeError && (error as any)?.message === 'Network request failed') {
          throw new ApiFetchError(
